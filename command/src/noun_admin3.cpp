...
int main(int argc, char **argv) {
  ...
  while(1){
	...
	if(robot == 3){
      ...
      ifstream stream3("/home/yhirai/catkin_ws/src/verb/src/verb_admin.json");
      int is;
      stream3 >> root;
      const picojson::value::object& obj = root.get<picojson::object>();
      for (picojson::value::object::const_iterator i = obj.begin(); i != obj.end(); ++i) {
	json_map.insert( make_pair(i->first,i->second.to_str() ) );
      ...
      for(is = 1; is < noun_msg->value ; is++){
	c = json_map.count(noun_msg->noun[is]);
	if(c != 0){
	  object image = root.get<object>()[noun_msg->noun[is]].get<object>();
	  string hh = image["command"].get<string>();
	  if(hh == "where"){
	    string name_full;
	    int num_name = 0;
	    for(int is2 = 0 ; is2 < 12 ; is2++){
	      string url = "/home/yhirai/speech_dic/" + noun_msg->name + "/word/" + noun_msg->noun[0] + ".txt";
	      struct stat st;
	      const char* dir;
	      int ret;
	      dir = url.c_str();
		...
		ss << line;
		ss >> number_name;
		if(number_name>num_name){
		  num_name = number_name;
		  name_full = noun_msg->name;
		}
	      }
	    }
	    cout << name_full << "sangamottennjanaissuka" << endl;
	  }
	}
      }
    }
