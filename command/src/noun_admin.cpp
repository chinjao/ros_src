...
int main(int argc, char **argv) {
  ...
  std_msgs::Int32 command_int;
  std_msgs::String chat_msg;
  ros::init(argc, argv, "noun");
  ros::NodeHandle n;
  ros::Publisher command_pub = n.advertise<std_msgs::Int32>("chatter", 1000);
  ...
  while(1){
	...
	if(c > 0){
	  ...
	  string data2;
	  data2 = image["command"].get<string>();
	  command_int.data = atoi(data2.c_str());
	}
	robot = 1;
	...
	while(1){
	  ...
	  if(command_int.data == 0){
		...
		//	cout << "command=" << image["command"].get<string>() << endl;
		string data3 = image3["command"].get<string>();
		command_int.data = atoi(data3.c_str());
	      }
	    }
	    if(command_int.data == 0){
