...
int main(int argc, char **argv){
  ...
  ros::NodeHandle n;
  ros::Publisher noun_pub = n.advertise<zmq_ros::amivoice_noun_verb2>("noun", 1000);
  zmq_ros::amivoice_noun_verb2 noun_msg;
  while(1){
    zmq_ros::mecab_amivoice2ConstPtr msg = ros::topic::waitForMessage<zmq_ros::mecab_amivoice2>("mecab_res");
    int i = 1;
    int d = 1;
	...
	if(msg->part[i] == "名詞"){
	  ...
	  int ret;
	  const char* text;
	  textfile = "/home/yhirai/speech_dic/" + msg->name + "/word/" + msg->word[i] + ".txt";
	  text = textfile.c_str();
	  ret = stat(text,&st);
	  if(ret != 0){
	    ofstream ofs(textfile.c_str());
	    ofs << 1 << endl;
	  }
	  else{
