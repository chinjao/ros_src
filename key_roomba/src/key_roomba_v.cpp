...
void TBK_Node::keyboardLoop()
{
  ...
  for(;;)
	...
	continue;
      */
      if(strcmp("Go",kk) == 0 && strcmp(before,kk) != 0){//KEYCODE_I){
  	  speed = 1;
	  turn = 0;
	  dirty = true;
	  system("sh ~/openjtalk/talk.sh \"まえにいどうします\"");
      }
      else if(strcmp("Left",kk) == 0 && strcmp(before,kk) != 0){  //KEYCODE_J:
	speed = 0;
	turn = 1;
	dirty = true;
	//	cout << "Hirai" << endl;
	system("sh ~/openjtalk/talk.sh \"ひだりにかいてんします\"");
      }
      else if(strcmp("Right",kk) == 0 && strcmp(before,kk) != 0){// KEYCODE_L:
	speed = 0;
	turn = -1;
	dirty = true;
	system("sh ~/openjtalk/talk.sh \"みぎにかいてんします\"");
      }    
      else if(strcmp("Back",kk) == 0 && strcmp(before,kk) != 0){ //KEYCODE_COMMA:
	speed = -1;
	turn = 0;
	dirty = true;
	system("sh ~/openjtalk/talk.sh \"うしろにいどうします\"");
      }
      else if(strcmp("Stop",kk) == 0 && strcmp(before,kk) != 0){
	speed = 0;
	turn = 0;
	dirty = true;
