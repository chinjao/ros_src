...
public:
  ...
  void browser(speech_msgs::SpeechConstPtr speech,int okao_id){
	...
	if(size_s - i >= 33){
	  ...
	  dict["time"] = speech->time;
	  dict["place"] = "pioneer";
	  dict["speechRec"] = speech->sentence.substr(i , i + 33);
	  cout << dict["speechRec"] << endl;
	}
	else{
	  dict.clear();
	  dict["senderName"] = "speechRecognition";
	  ...
	  dict["time"] = speech->time;
	  dict["place"] = "pioneer";
	  string str_sp = speech->sentence.substr(i,size_s-i);
	  dict["speechRec"] = str_sp;
	  cout << dict["speechRec"] << endl;
	}
	msgpack::sbuffer buffer;
	msgpack::pack(buffer,dict);
  ...
  void command(speech_msgs::Extract ext_msg,int okao_id){
    ...
    ros::Publisher chat_pub = nh.advertise<std_msgs::String>("julius", 100);
    string aa;
    string bb = "なし";
    int robot = 0;
    int sub_robot = 0;
