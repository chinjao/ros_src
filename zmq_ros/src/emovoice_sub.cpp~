...
struct speech{
  ...
  double negative;
  double normal;
  MSGPACK_DEFINE(tracking_ID,speech_dictation,emotion, day_time,month,day,hour,minute,second,place,positive,negative,normal);
}speech_dic;

int main(int argc, char **argv){
  ros::init(argc, argv, "emotion_pub");

  ros::NodeHandle n;
  struct stat st;
  const char* dir;
  int ret;
  ros::Publisher zmq_pub = n.advertise<speech_msgs::Speech>("emotion_info", 1000);
  ...
  while(1){
	...
	mkdir(dir,0777);
      ...
      zmq_msg.positive = speech_dic.positive;
      zmq_msg.negative = speech_dic.negative;
      zmq_msg.normal = speech_dic.normal;
      cout << zmq_msg.normal << endl;
      //zmq_msg.sentence = speech_dic.speech_dictation;
      zmq_msg.emotion = speech_dic.emotion;
      zmq_pub.publish(zmq_msg);
      cout << "音声認識結果:" << speech_dic.emotion << endl;
      cout << "取得時間:" << speech_dic.day_time << endl;
      cout << "取得場所:" << speech_dic.place << endl;
      cout << speech_dic.tracking_ID << endl;
      cout << endl;
  }
  return 0;
}
