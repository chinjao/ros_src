...
public:
  ...
  void browser(speech_msgs::SpeechConstPtr speech,int okao_id){
    ...
    emo_hi = "po:" + to_string(posi) + ",ne:" + to_string(nega) + ",no:" + to_string(norm);
    dict["place"] = emo_hi;
    if(speech->emotion == "positive"){
      cout << "positive point: " << "positive" << endl;
	dict["mood"] = "255";
	dict["speechRec"] = "positive";
    }
    else if(speech->emotion == "negative"){
      dict["mood"] = "-255";
      dict["speechRec"] = "negative";
    }
    else if(speech->emotion == "normal"){
      dict["mood"] = "0";
  ...
  void speech_pub(speech_msgs::SpeechConstPtr speech,int okao_id){
    speech_msgs::Emotion emo_msg;
    string name = TIDtoName(okao_id);
    emo_msg.name = name;
    emo_msg.emotion = speech->emotion;
    emo_msg.positive = speech->positive;
    emo_msg.negative = speech->negative;
    emo_msg.normal = speech->normal;
    ros::Publisher sp_pub = nh.advertise<speech_msgs::Emotion>("emo_and_name",1000);
    sp_pub.publish(speech);
  }
