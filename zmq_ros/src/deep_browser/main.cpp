...
public:
  ...
  string TIDtoName(int okao_id){
    string name;
    string vec[] = {"Uema","Haruta","Haruna","Ikegami","Adachi","Hirai","Teranishi","Kawakita","Fukuhara","Yano","Wakimoto","Yatsuduka","Shimada","Mukai","Yamamoto"};
    /*for(int h_i = 0; h_i < human_last.size() ; ++h_i)
      {*/
    cout << "now" << endl;
    if(okao_id != 0 && okao_id < 16){
      int acc = okao_id - 1;
      name = vec[acc];
    }
  ...
  void browser(speech_msgs::SpeechConstPtr speech,int okao_id){
    ...
    sender.connect("tcp://133.19.23.114:8081");
    map<string, string> dict;
    map<string, MultiData> resultpack;
    resultpack["senderName"] = MultiData("emotion", "string", "senderName");
    cout << "in" << endl;
    dict["senderName"] = "speechRecognition";
    string name;
    name = TIDtoName(okao_id);
    if(okao_id == 6 && name != "Hirai"){
      name = "Hirai";
      cout << "okasiyo" << endl;
    }
    cout<<"name:"<<name <<endl;
    ...
    const char* ht[] = {"Angry","Disgust","Fear","Happy","Sadness","Surprise","Normal"};
    vector<string> emotion(begin(ht), end(ht));

    for (int idxEmotion = 0; idxEmotion < 7; ++idxEmotion){
      string key = "emotion" + toString(idxEmotion); // キー作成
      double v = bayes_msg->estimation_value[idxEmotion]; //乱数生成
      // 送信用データに追加
      resultpack[key] = MultiData(toString(v), "string", emotion[idxEmotion]); // 第3引数にはkeyではなく感情の名称の文字列を入れても良いかも
    }
    msgpack::sbuffer buffer, buffer2;
    msgpack::pack(buffer,dict);
    msgpack::pack(buffer2,resultpack);
       
    zmq::message_t message_send2(buffer2.size());
    memcpy(message_send2.data(), buffer2.data(), buffer2.size());
    // 可視化サーバに送信
    sender.send(message_send2);
  ...
  void speech_pub(speech_msgs::SpeechConstPtr speech,int okao_id){
    ...
    for(int var = 0; var < 7 ; var++)
      emo_msg2.observation_value[var] = speech->observation_value[var];
    /*
