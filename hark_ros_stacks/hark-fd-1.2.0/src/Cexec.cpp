...
int Cexec :: InitTFMatrix(MICARY_PrmFrame *psPrmFrame)
{
	...
	iUpper_f = m_psSepMat->micary_smSeparation.micary_iaFreqLinex.ha_numValue +iLower_f -1 ;
		...
		/*
		std::vector<float> allHeight(psTFDBHead->lNSrc); //高さを格納

		//TFのデータから高さの数，水平方向のポイント数を取得
		for(unsigned int i = 0; i < psTFDBHead->lNSrc; i++)
		{
			allHeight[i] = m_psTFDB->micary_locaMic.micary_Location[i].ha_fZ ;
		...
		psTFDBHead->ha_numHieght = numHeight;
		psTFDBHead->ha_numDirection = floor(psTFDBHead->lNSrc / numHeight);
...
// ----------------------------------------------------------------------------------
bool Cexec::SetPrmUpdate(int iUpdateMTF_CONJ, int iUpdateMW, float fThred_angle, float fSearch_azimuth ,float fSearch_elevation)
{
...
/*
	HA_Float fX,fY,fZ,fTheta,fPhi,pi ;
	pi = acos(-1.0) ;
	fPhi = 16.7 *pi/180.0;

	for(ii = 0 ; ii < iNLoc ; ii++){
		fTheta = ii*__SAVED_LOC_THETA_PITCH*pi/180.0 ;
		fX = cos(fTheta)*cos(fPhi) ;
		fY = sin(fTheta)*cos(fPhi) ;
		fZ = sin(fPhi) ;

		m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fX = fX ;
		m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fY = fY ;
		m_updateW->micary_dbSeparation.micary_locaSource.micary_Location[ii].ha_fZ = fZ ;
	}
*/
...
bool Cexec::_IsThreshPos(MICARY_Location laRefPos,MICARY_Location laObjPos)
{
	...
	//// 許容範囲の内積を計算
	//MICARY_Location laThresOrg,laThresAng ;
	//float fPOrg ,fPObj;
	//
	//// 原点0°
	//laThresOrg.ha_fPhi = 16.0 ;
	//laThresOrg.ha_fTheta = 0.0 ;
	//_Ang2Pos(&laThresOrg) ;

	//// 許容角度
	//laThresAng.ha_fPhi = 16.0 ;
	//laThresAng.ha_fTheta = m_fThred_angle ; 
	//_Ang2Pos(&laThresAng) ;

	//// 許容角度の内積
	//fPOrg = _InnerProductLoc(laThresOrg,laThresAng) ;

	// 目的角度の内積を計算
	fPObj = _InnerProductLoc(laRefPos,laObjPos) ;
	//cout<<"laRefPos,"<<laRefPos.ha_fX<<","<<laRefPos.ha_fY<<","<<laRefPos.ha_fZ<<endl;
	//cout<<"laObjPos,"<<laObjPos.ha_fX<<","<<laObjPos.ha_fY<<","<<laObjPos.ha_fZ<<endl;
	//cout<<"fPObj,"<<fPObj<<endl;
	//cout<<"abs(acos(fPObj)),"<<180/acos(-1.0)*abs(acos(fPObj))<<endl;

	if(fPObj>1 && fPObj<1.001){ //内積1に相当
	//	cout<<"threash_true"<<endl;
		return true ;
	}
