...
class Compmat{
	long            nrow, ncol, nrowcol;
	complex<float> *zval;
public:
	Compmat();
	Compmat(const long& Nrow);
	Compmat(const long& Nrow, const long& Ncol);
	...
	Compmat& operator&=(const Compmat&);
	Compmat operator|(const Compmat&); // MATLAB (*this)./X with repetition of X
	complex<float>& operator()(const long& row, const long& col) const {return zval[row+col*nrow];}
	complex<float>& operator()(const long& rowcol)               const {return zval[rowcol];}
	Compmat operator()(const Compmat&) const; // Read only (A(B) = C is not OK)
	void zeros(const long& Nrow, const long& Ncol);
	void zerossize(const Compmat& x){zeros(x.size1(), x.size2());}
	long size1() const {return nrow;}
	long size2() const {return ncol;}
	long size()  const {return nrowcol;}
	long end1() const {return nrow-1;}
	long end2() const {return ncol-1;}
	long end()  const {return nrowcol-1;}
	float& r(const long& row, const long& col)          const {return *((float*)&zval[row+col*nrow]);}
	float& r(const long& rowcol)                        const {return *((float*)&zval[rowcol]);}
	float& i(const long& row, const long& col)          const {return *((float*)&zval[row+col*nrow] + 1);}
	float& i(const long& rowcol)                        const {return *((float*)&zval[rowcol] + 1);}
	Compmat mid1(long, long) const; // Y = (*this)(start:end, :)
	Compmat mid1(const long& row) const {return mid1(row, row);} // Y = (*this)(row, :)
	Compmat mid2(long, long) const; // Y = (*this)(start:end, :)
	Compmat mid2(const long& col) const {return mid2(col, col);} // Y = (*this)(:, col)
	Compmat norm2() const;
	void hanning(const long&);
	void disp() const;
	...
	Compmat reshape(const long&, const long&) const; // reshape with automatic truncation or 0 padding
	Compmat reshape(const long& Nrow) const {return reshape(Nrow, nrow*ncol/Nrow);}; // Nlow ommit version
	...
	void    TBL_addset_tr(const Compmat&, const short*, const long&, const float&);
	Compmat repmat0(const long&, const long&) const; // repmat with allow non-integer repeat
	Compmat repmat(const long& NrowR, const long& NcolR) const{return repmat0(nrow*NrowR, ncol*NcolR);}; // MATLAB repmat
...
Compmat::Compmat(const long& Nrow)                   {nrowcol = 0; zeros(Nrow, 1);}
Compmat::Compmat(const long& Nrow, const long& Ncol) {nrowcol = 0; zeros(Nrow, Ncol);}
...
void Compmat::zeros(const long& Nrow, const long& Ncol){
	long Nrowcol;
	nrow = Nrow;
	ncol = Ncol;
	if(nrow < 1){nrow = 1;}
...
Compmat Compmat::mid1(long start, long end) const{
	Compmat y;
	...
	for(long n1 = 0; n1 < y.size1(); n1++){
		for(long n2 = 0; n2 < size2() ; n2++){
...
Compmat Compmat::mid2(long start, long end) const{
	Compmat y;
	...
	for(long n1 = 0; n1 < size1(); n1++){
		for(long n2 = 0; n2 < y.size2() ; n2++){
...
void Compmat::hanning(const long& Nrow){
...
Compmat Compmat::reshape(const long& Nrow, const long& Ncol) const{
	Compmat retval;
	long N;
...
//	High speed spec.
void Compmat::TBL_addset_tr(const Compmat& x, const short* TABLE, const long& SBIT, const float& tr){
	long  *ptemp;
	long  ltemp;
	long n1, n2, n12, N2, N12;
	float *rn12;
...
Compmat Compmat::repmat0(const long& Nrow, const long& Ncol) const{
	Compmat retval;
	long n1, n2, n1mod, n2mod;
	retval.zeros(Nrow, Ncol);
...
public:
	...
	Compmat fft0(const Compmat&, long, long);
	Compmat fft(const Compmat& x, long NFFT){return fft0(x, NFFT, -1);}
	Compmat ifft(const Compmat& x, long NFFT){return fft0(x, NFFT, 1);}
	void winfft(Compmat&, const Compmat&);
	void fft0ov(Compmat&, long); // size1() should be 2^N
};

Compmat Cfft:: fft0(const Compmat& x, long NFFT, long sign){
	...
	if(nfft != NFFT){
		...
		delete [] ip;
		nfft = NFFT;
		nfftsqrt = (long)sqrt((float)nfft);
		w = new float[nfft * 5 / 4];
		ip = new long[nfftsqrt + 2];
		ip[0] = 0;
	}
	X.zeros(nfft, x.size2());
	X += x;
	if(sign > 0){X *= 1/(float)nfft;}
	for(long n2 = 0; n2 < x.size2(); n2++){
		cdft((int)nfft*2, (int)sign, &(X.r(0, n2)), (int*)ip, w);
	}
	return X;
}

void Cfft:: fft0ov(Compmat& x, long sign){
	void cdft(int, int, float*, int*, float*);
	long NFFT = x.size1();
	if(nfft != NFFT){
		delete [] w;
...
class Wavdata {
	...
	Wavfmt    fmt;
	short     *data;
	long      dataptr; // pointer to DATA chank
	long      datasize; // size of DATA chank
	long      buffersize; // buffersize in bytes
	long	  cptr; // for BCC32's bug tellg() return wrongly	
	bool      err;
...
public:
	...
	void setframe(long);
	void read0(char* ptr, long size);
	long read(long);
	short& x(long n){return x(n, 0);}
	short& x(long n, long ch){return data[n+ch*(long)fmt.wChannels];}
	long nch() const {return (long)fmt.wChannels;}
	long nsmp() const {return datasize/sizeof(short)/(long)fmt.wChannels;}
	long nfs() const {return (long)fmt.dwSamplesPerSec;}
	bool fail() const {return err;}
	long bufsize() const {return buffersize;}
};
...
void Wavdata::setframe(long nframe){
	delete data;
...
long Wavdata::read(long startsample){
...
void Wavdata::read0(char* ptr, long size){
	ifs.read(ptr, size);
	cptr += ifs.gcount();
}
...
class CWavdata : public Wavdata{
	Compmat   cdata;
	long      framesize; // frame in time samples
public:
	CWavdata() : framesize(0) {};
	void setframe(long);
	Compmat& read(long);
	Compmat& read(long, long);
	Compmat& cx(){return cdata;}
};

void CWavdata::setframe(long nframe){
	framesize = nframe;
	Wavdata::setframe(nframe);
	cdata.zeros(framesize, nch());
}

Compmat& CWavdata::read(long startsample){
    Wavdata::read(startsample);
	for(long ch = 0; ch < nch(); ch++){
		for(long t = 0; t < framesize; t++){
			cdata.r(t, ch) = x(t, ch);
		}
	}
	return cdata;
...
Compmat& CWavdata::read(long startsample, long nframe){
...
public:
	Wavdata2();
	Wavdata2(const char *fname, long nfs){openw(fname, nfs);}
	void openw(const char* fname){openw(fname, 16000);}
	void openw(const char* fname, long nfs);
	long nch() const {return (long)ofmt.wChannels;}
	long nfs() const {return (long)ofmt.dwSamplesPerSec;}
	long nsmp(long);
	long nsmp();
	void fmtwrite(long, long);
	void fmtwritep(long);
	void write0(short*, long, long, long);
	void write0(short*, long, long);
};
...
void Wavdata2::openw(const char* fname, long nfs){
	ofname = fname;
	ofmt.dwSamplesPerSec = nfs;
}

void Wavdata2::fmtwrite(long Nch, long Nsmp){
	long ltmp;
	...
	ofmt.dwSamplesPerSec = (unsigned long)nfs(); // #fs
	ofmt.dwAvgBytesPerSec = (unsigned long)(nfs()*Nch*sizeof(short)); //#byte/sec
...
void Wavdata2::write0(short* wdata, long startsample, long length, long Nch){
	if(ofname == ""){return;}
...
void Wavdata2::write0(short* wdata, long startsample, long length){
	long Nch;
	Nch = ofmt.wChannels;
	if(Nch == 0){Nch = 1;}
...
void Wavdata2::fmtwritep(long Nch){
...
public:
	...
	void setframe(long);
	void write(Compmat, long);
	short& x(long n){return data[n];}
};

void CWavdata2::setframe(long sbsize){
...
void CWavdata2::write(Compmat cmat, long startsample){
	long sbsize, s1, s2, s1t, s2t,lcnt;
...
// CLASS: HPost4
// -----------------
const long UPDATE_TABLE_SIZE = 32768;
const long UPDATE_TABLE_SBIT = 16;
const long UPDATE_TABLE_STEP = (long)1 << UPDATE_TABLE_SBIT;
...
void HPostv4::initialize(const long& NfreqNch){
	long n, m;
    void *vp;
	Compmat ctmp, mind;
	float*tpflt;
