 ...
 * @parameter_type string
 * @parameter_value DIAG
 * @parameter_list FULL:DIAG
 * @parameter_description The calculation method for geometric constraints. "FULL" uses all elements of a matrix, and "DIAG" only uses diagonal parts. [default: DIAG]
 *
...
public:
	...
	if(parameters.exist("SS_SCAL")){
	    ...
	    */
	    ...
	    m_cGHDSSExec.SetPrmUpdate(iUpdate_TF_CONJ,iUpdate_W,fUpdate_angle,fsearch_azimuth,fsearch_elevation) ;
...
//Output_txt			
			//cout<<"input_sng"<<i<<","<<locaInputSrcPos.micary_Location[i].ha_fTheta<<","<<locaInputSrcPos.micary_Location[i].ha_fPhi <<endl;		
			//locaInputSrcPos.micary_Location[i].ha_fPhi = 180.0/acos(-1.0)*atan2((double)src->x[1],(double)src->x[0]) ;			
			//locaInputSrcPos.micary_Location[i].ha_fTheta = -180.0/acos(-1.0)*asin((double)src->x[2]) ; 
		...
		//Changed by Morio 2013/01/23 GHDSSでのFixed Noise廃止
		// fixednoiseのチェック
	  //  if(fixed_noise){
			//if(!m_cGHDSSExec.CheckFixedNoise_noise(locaInputSrcPos)){ //検出されていないならば座標情報の追加
			//    m_cGHDSSExec.MakeFixedNoise_Pos(locaInputSrcPos);
			//}
	  //  }
			// ソート
		...
		if(m_bFlgIni == true){
			...
			throw(new NodeException(NULL, "Exception caught in InitTFMatrix calculate[GHDSS]", __FILE__, __LINE__));
			return ;
		    }
